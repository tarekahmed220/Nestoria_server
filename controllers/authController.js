...
const signToken = (id, role) => {
  return jwt.sign({ id, role }, "furnitureapp", { expiresIn: "100d" });
};
...
const login = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    ...
    return next(
      new AppError("This email does not exist. Please sign up instead", 422)
    );
  }
...
const signup = catchAsync(async (req, res, next) => {
  ...
  if (userfound) {
    return next(
      new AppError("This account already exists, please sign in instead!", 422)
    );
  }
  ...
  await user.save();
  const token = jwt.sign({ email: user.email }, "furnitureapp", {
    expiresIn: "100d",
  }); // sign
...
const forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetURL = `${req.protocol}://${req.get(
    "host"
  )}/api/v1/ng/auth/resetPassword/${resetToken}`;
